...
# Root-MUSIC 2 sources. 6 degrees (SNR = 10dB, N = 8)
root_music_locations = []
power_noise = power_source / (10 ** (10 / 10))
ula = model.UniformLinearArray(n_antenna_array, d0)
...
fig = plt.figure(figsize=(8, 8))
ax2 = fig.add_subplot()
ax2.scatter(estimates.locations, arr_of_ones, marker='o', color='red', s=60, label='N = 8 Elements, SNR = 10dB')
ax2.scatter(np.linspace(-np.pi / 32, 0, 2) * 180 / np.pi, arr_of_ones, marker='x', color='black', s=20, label='Real Source Places')
ax2.legend(loc='best')
ax2.set_xlabel('DOA/deg')
fig.gca().set_ylabel(r'P($\theta$)')
